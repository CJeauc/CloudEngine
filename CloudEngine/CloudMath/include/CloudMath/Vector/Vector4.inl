...
{
	template <typename T>
	constexpr Vector4<T> Vector4<T>::Zero() { return Vector4<T>(0, 0, 0, 1); }

	template <typename T>
	constexpr Vector4<T> Vector4<T>::XAxis() { return Vector4<T>(1, 0, 0, 1); }

	template <typename T>
	constexpr Vector4<T> Vector4<T>::YAxis() { return Vector4<T>(0, 1, 0, 1); }

	template <typename T>
	constexpr Vector4<T> Vector4<T>::ZAxis() { return Vector4<T>(0, 0, 1, 1); }
...
#pragma region Constructors & Assignment
	...
	constexpr Vector4<T>::Vector4(Vector4<T>&& p_other) noexcept
		: x{std::move(p_other.x)}, y{std::move(p_other.y)},
		  z{std::move(p_other.z)}, w{std::move(p_other.w)} { }
...
#pragma region Tests & Comparisons
	...
	{
		Vector4<T> normThis = Vector4<T>::Normalize(*this);
		Vector4<T> normOther = Vector4<T>::Normalize(p_other);
...
#pragma region Vector Operations
	...
	{
		return (static_cast<T>(MathTools::MathUtils::SquareRootF(x * x + y * y + z * z)));
	}
...
#pragma region Conversions
	...
	template <typename T>
	constexpr Vector3<T> Vector4<T>::ToVector3() const
	{
		const float inverseW = 1 / w;
		return Vector3<T>(static_cast<T>(x * inverseW),
		                  static_cast<T>(y * inverseW),
		                  static_cast<T>(z * inverseW));
	}

	template <typename T>
	constexpr Vector3<T> Vector4<T>::ToVector3(const Vector4<T>& p_vector4)
	{
		return p_vector4.ToVector3();
	}
...
#pragma region Outside Operators
	...
	template <typename T>
	constexpr std::ostream& operator<<(std::ostream& p_stream, const CloudMath::Vector4<T>& p_vector)
	{

...
{
	template <typename T>
	constexpr Vector3<T> Vector3<T>::Zero() { return Vector3<T>(0, 0, 0); }

	template <typename T>
	constexpr Vector3<T> Vector3<T>::XAxis() { return Vector3<T>(1, 0, 0); }

	template <typename T>
	constexpr Vector3<T> Vector3<T>::YAxis() { return Vector3<T>(0, 1, 0); }

	template <typename T>
	constexpr Vector3<T> Vector3<T>::ZAxis() { return Vector3<T>(0, 0, 1); }
...
#pragma region Constructors & Assignment
	...
	constexpr Vector3<T>::Vector3(Vector2<T>&& p_other)
		: x{std::move(p_other.x)}, y{std::move(p_other.y)}, z(0) {}
	...
	constexpr Vector3<T>::Vector3(Vector3<T>&& p_other) noexcept
		: x{std::move(p_other.x)}, y{std::move(p_other.y)}, z{std::move(p_other.z)} { }
	...
	template <typename T>
	constexpr Vector3<T>& Vector3<T>::operator=(Vector3<T>&& p_other) noexcept
	{
...
#pragma region Tests & Comparisons
	...
	{
		Vector3<T> normThis = Vector3<T>::Normalize(*this);
		Vector3<T> normOther = Vector3<T>::Normalize(p_other);
...
#pragma region Outside Operators
	...
	template <typename T>
	constexpr std::ostream& operator<<(std::ostream& p_stream, const CloudMath::Vector3<T>& p_vector)
	{
